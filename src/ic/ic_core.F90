...
PROGRAM ic_core
  ...
  real(kind=PR) :: mtot             ! Mass
  real(kind=PR) :: old_mtot         ! ..
  real(kind=PR) :: pofr             ! ..
  real(kind=PR) :: r1               ! ..
  real(kind=PR) :: r2               ! ..
  real(kind=PR) :: radmax           ! ..
  ...
  if (coretype == "plummer") then
     ...
     do        
        ...
        do i=1,nmax
           ...
           if (coretype == "plummer") then
              mtot = mtot + 0.5_PR*(r2 - r1)*(fmplummer(r1,rcore,rho0,pofr) &
        ...
        write(6,*) "Iteration : ",nmax,mtot,abs((mtot - old_mtot)/mtot)
        if (abs((mtot - old_mtot)/mtot) < 1.0E-8_PR) exit
...
! ============================================================================
     ...
     do i=1,nmax
        ...
        r2            = radmax*real(i,PR)/real(nmax,PR)
        radius(i)     = r2
        mtot          = mpowerlaw(r2,rcore,rho0,pofr)
...
! ============================================================================
     ...
     nmax = 10000
     rho0 = 3.0_PR*mtot/(4.0_PR*PI*(radmax**3))
     ...
     do i=1,nmax
        ...
        r2            = radmax*real(i,PR)/real(nmax,PR)
        radius(i)     = r2
        mtot          = 4.0_PR*PI*rho0*(r2**3)/3.0_PR
        density(i)    = rho0
        mass_array(i) = mtot
     end do
     ! -----------------------------------------------------------------------
...
! ============================================================================
  ...
  write(6,*) "Mass of cloud     : ",mtot*mscale,munit
...
! Sort particles into gas, icm and boundary zones
  ...
  rgas      = ricm*((mtot + menvelope)/&
       &(mtot + menvelope + micmenv))**(ONETHIRD)
  rcloud    = ricm*(mtot/(mtot + menvelope + micmenv))**(ONETHIRD)
...
! ----------------------------------------------------------------------------
  mp = (mtot + menvelope) / real(pgas,PR)

  do p=1,ptot
     call distance2(rcentre(1:NDIM),p,dr(1:NDIM),drsqd)
     drmag = sqrt(drsqd) + SMALL_NUMBER
     if (drmag >= rcloud) then
        radp = (3.0_PR*mtot)/(4.0_PR*PI*density(nmax))&
             &*((drmag/rcloud)**3 - 1.0_PR) + radmax**3
        radp = radp**(ONETHIRD)
        parray(1:NDIM,p) = parray(1:NDIM,p)*radp/drmag
     else
        mint = mtot*(drmag/rcloud)**3
        ip = 1
        do i=1,nmax
           ip = i
...
! Write particle data to file
  ...
  renvelope = ((3.0_PR*mtot)/(4.0_PR*PI*density(nmax))&
       &*((rgas/rcloud)**3 - 1.0_PR) + radmax**3)**(ONETHIRD)
  ricm      = ((3.0_PR*mtot)/(4.0_PR*PI*density(nmax))&
       &*((ricm/rcloud)**3 - 1.0_PR) + radmax**3)**(ONETHIRD)
  rboundary = ((3.0_PR*mtot)/(4.0_PR*PI*density(nmax))&
