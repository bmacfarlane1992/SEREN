...
! Record original particle positions in temporary array
  ...
  do p=1,ptot
     position(1:NDIM,p) = parray(1:NDIM,p)
  end do
...
! Create nrepeat**(NDIM) cubes and resize to fit from 0 to 1.
  ...
  do p=1,old_ptot
     do i=1,nrepeat
        do j=1,nrepeat
           iaux = iaux + 1
           parray(1,iaux) = (position(1,p) + real(i,PR) - 1.0_PR) &
                & / real(nrepeat,PR)
           parray(2,iaux) = (position(2,p)+real(j,PR)-1.0_PR) &
                & / real(nrepeat,PR)
        end do
     end do
  end do
     ...
     do i=1,nrepeat
        do j=1,nrepeat
           do k=1,nrepeat             
              ...
              parray(2,iaux) = (position(2,p) + real(j,PR) - 1.0_PR) &
                   & /real(nrepeat,PR)
              parray(3,iaux) = (position(3,p) + real(k,PR) - 1.0_PR) &
                   & /real(nrepeat,PR)
           end do
        end do
     end do
