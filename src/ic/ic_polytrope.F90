...
PROGRAM ic_polytrope
  use interface_module, only : distance2,paramerror,paramstore,read_data,write_data
  use particle_module
  use filename_module
  use scaling_module
  use time_module
...
! Check parameter file is consistent with chosen EOS.
  ...
  if (.not. isocloud) stop 'Incorrect EOS or incorrect value of isocloud'
#elif defined(POLYTROPIC)
  if (isocloud) stop 'Incorrect EOS or incorrect value of isocloud'
#elif !defined(INTERNAL_ENERGY)
  stop 'Incompatable EOS'
#endif
...
! ----------------------------------------------------------------------------
     ...
     mu      = mu_array(ibound)
     mpoly   = mpoly / mscale
     rho0    = (4.0_PR*PI*(Kpoly*(npoly + 1)/(4.0_PR*PI))**&
          &(1.5_PR)*mu/mpoly)**(2*npoly/(npoly - 3))
...
! Calculate thermal properties
  do p=1,ptot
     press(p) = Kpoly*rho(p)**gamma
     temp(p) = press(p)/rho(p)/Pconst
#if defined(INTERNAL_ENERGY)
     if (isocloud) then
        u(p) = 1.5_PR*press(p)/rho(p)
     else
        u(p) = press(p)/rho(p)/(gamma - 1.0_PR)
     end if
#endif
  end do
