...
!
 real(kind=PR) :: x_amr, y_amr, z_amr, vx_amr, vy_amr, vz_amr	!!! Data that will be printed to amr_grid.py
 real(kind=PR) :: mass_amr, h_amr, temp_amr						!!! program to convert SPH->AMR format for RADMC- 3D
!
 real(kind=DP), parameter :: delt_r = 4.		!!! Define parameters for PV
 real(kind=DP), parameter :: delt_v = 0.25		!!! diagram
 real(kind=DP), parameter :: r_range = 200.
 real(kind=DP), parameter :: v_range = 60.
 real(kind=DP), parameter :: y_min = -20.
 real(kind=DP), parameter :: y_max = 20.
!
 ...
 real(kind=PR), allocatable :: pv_mass(:)
 real(kind=PR), allocatable :: pv_temp(:)
 integer :: rint, r_iter, vint, v_iter, r_count, v_count, hist_ind, hist_count
 real(kind=PR) :: bin_temp
...
!
 										!!! Choose whether (1) or not (0) to do following options

 integer :: amr_print = 0				! Print AMR inputs
 integer :: pv_print = 1				! Generate PV data
 integer :: rdisc_print = 0				! Generate rdisc.1.dis data
 integer :: pdisc_print = 0				! Generate pdisc.1 data
 integer :: rdisc_crit_print = 0		! Generate rdisc.1 data
 integer :: cdisc_print = 0				! Generate cdisc data
...
!					!!! either from file or in this script as defined in script_params
 if (script_params .eq. 1) then
	open(unit = 11, file = "/Users/bmacfarlane1992/Documents/ACADEMIA_PHD/PROJECTS/SEREN_GENERATE/varfile.dat", &
	   status = "old")
...
!collective files with what happens in a given radius vs time (appends on file)  rdisc.x.dis
  ...
  write(1,'(16E15.7,I7,2X,18A)') time*tscale*1e6,radius(i)*206265,rtoomre(i),&
			...
			& sink(1)%Mstar*real(mscale,PR), 0.0,&
			& rv_r(i)*real(vscale,PR),rv_theta(i)*real(vscale,PR),rv_z(i)*real(vscale,PR),&
			& sqrt(((rinmass(i)+sink(nsin)%Mstar)/real(mscale,PR))/(radius(i)))*real(vscale,PR),& ! #17
!			& sqrt(((rinmass(i)+sink(nsin)%Mstar+sink(nsin)%Mdisc)/real(mscale,PR))/(radius(i)))*real(vscale,PR),& ! #17
			& smooth_av(i)*206265, smooth_midplane_av(i)*206265, &!# 18, 19
			&rparticles(i),&
			& trim(adjustl(in_file))
...
!
! Output AMR grid inputs to be processed in amr_grid.py
!
 if (amr_print .eq. 1) then
!
	outfile_histpv = trim(adjustl(in_file))//"."//trim(adjustl(out_file_form))//".amr_ascii."//trim(adjustl(file_ext3))
	open(UNIT=1,file=outfile_histpv,form='formatted',position='append')
	do p=1,ptot
		if( (rho(p)*rhoscale .gt. rhorestr) .or. (temp(p) .lt. trestr) ) then
			x_amr = parray(1,p)*rscale*206265.
			y_amr = parray(2,p)*rscale*206265.
			z_amr = parray(3,p)*rscale*206265.
			vx_amr = v(1,p)*vscale
			vy_amr = v(2,p)*vscale
			vz_amr = v(3,p)*vscale
			mass_amr = parray(MASS, p)*real(mscale,PR)
			h_amr = parray(SMOO,p)
			temp_amr = temp(p)
			write(1, '(9E15.7, 9X)') x_amr, y_amr, z_amr, vx_amr, vy_amr, vz_amr, mass_amr, h_amr, temp_amr
		endif
	enddo
	close(1)
endif
!
! Output PV diagram data for raw data, histogram-ed data and x-y points for Keplerian fit
!
 if (pv_print .eq. 1) then
!
	allocate(pv_x(1:trho_count))
	allocate(pv_y(1:trho_count))
	allocate(pv_vz(1:trho_count))
	allocate(pv_mass(1:trho_count))
	allocate(pv_temp(1:trho_count))
!
	i = 1
	do p=1,ptot
		if( (rho(p)*rhoscale .gt. rhorestr) .or. (temp(p) .lt. trestr) ) then
			...
			pv_mass(i) = parray(MASS, p)*real(mscale,PR)
			pv_temp(i) = temp(p)
			i = i + 1
		endif
	enddo
...
!
	...
	write(2, '(E15.7)') y_min
	write(2, '(E15.7)') y_max
	write(2, '(E15.7)') ( sum(pv_mass) / float(trho_count) )
!
			...
			do i=1, trho_count
				if ( ( pv_x(i) .gt. r_arr1(rint) ) .and. ( pv_x(i) .lt. r_arr2(rint) ) &
				   ...
				   .and. (pv_y(i) .gt. y_min ) .and. (pv_y(i) .lt. y_max ) ) then
					...
					bin_temp = bin_temp + pv_temp(i)
				endif
			enddo
!
			if (hist_count .ne. 0) then
			   bin_temp = bin_temp / float(hist_count)
			endif
!
			r_cent = ( r_arr1(rint) + r_arr2(rint) ) / 2.
			v_cent = ( v_arr1(vint) + v_arr2(vint) ) / 2.
!
			write(2, '(3E15.7, 2X, I7)') r_cent, v_cent, bin_temp, hist_count
!
			if ( (hist_count .ne. 0) .and. (r_arr1(rint) .le. 0.) ) then
				write(3, '(3E15.7, 2X, I7)') r_cent, v_cent, bin_temp, hist_count
			endif
!
			if ( (hist_count .ne. 0) .and. (r_arr1(rint) .ge. 0.) ) then
				write(4, '(3E15.7, 2X, I7)') r_cent, v_cent, bin_temp, hist_count
			endif
!
		enddo
	enddo
