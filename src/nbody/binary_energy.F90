...
#ifndef N_BODY
  integer :: kern1                         ! 1st kernel table element
  integer :: kern2                         ! 2nd kernel table element
  real(kind=DP) :: skern                   ! 0.5 * (r / hp ) * KERNTOT
#endif
...
! stars (as opposed to other binary systems). 
  if (s1 <= stot .and. s2 <= stot .and. &
     ...
     skern = HALFKERNTOT*drmag/h1
     kern1 = int(skern)
     kern1 = min(kern1,KERNTOT)     
     skern = HALFKERNTOT*drmag/h2
     kern2 = int(skern)
     kern2 = min(kern2,KERNTOT)     
     binen = 0.5_DP*(reduced_mass*dvsqd - m1*m2*(w6(kern1)/h1 + w6(kern2)/h2))
  else
